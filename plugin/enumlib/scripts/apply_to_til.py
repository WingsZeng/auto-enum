...
from enumlib.ui_plugins.ida_plugin import get_or_add_enum
...
class ida_opener:
    ...
    def __enter__(self):
        self.opened = idapro.open_database(str(self.idb), False)
        return self.opened
...
def process_til(
    ...
    with ida_opener(idb) as opened:
        ...
        auto_enum.enums.__getitem__.cache_clear()

        changed_types_count = 0

        for name in auto_enum.functions:
            changed_types_count += change_type(til, name, name, BOOL, auto_enum)
            # Add A/W versions for windows
            if platform_type == "windows":
                changed_types_count += change_type(til, name, name + "A", BOOL, auto_enum)
                changed_types_count += change_type(til, name, name + "W", BOOL, auto_enum)
            else:
                changed_types_count += change_type(til, name, "_" + name, BOOL, auto_enum)
...
def main():
    ...
    auto_enum = AutoEnum(args.platform)
